...
@Component({
    ...
    styles: ``,
    imports: [ CommonModule, RouterModule, ButtonLinkComponent, ReactiveFormsModule, IconLinkedinComponent, IconYoutubeComponent, IconInstagramComponent, IconFacebookComponent, IconDirective, ButtonComponent]
})
    ...
    this.newsletterService.subscribeToNewsletter(this.newsletterForm.getRawValue()).subscribe({
      next: (res: IApiResponse) => {
        if(res.success) {
          ...
          this._snackBar.open(res.errors['email'][0], '✖', {panelClass: 'failure-snackbar'});
        }
